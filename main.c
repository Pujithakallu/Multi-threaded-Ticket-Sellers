...
int main(int argc, char** argv) {
	...
	while(1) {
		pthread_mutex_lock(&thread_count_mutex); // locks the mutex object referenced by mutex.
		if(thread_count == 0) {
	...
	//Simulate each time quanta as one iteration
	...
	//Wakeup all thread so that no more thread keep waiting for clock Tick
	wakeup_all_seller_threads();
...
void wait_for_thread_to_serve_current_time_slice(){ //Check if all threads has finished their jobs for this time slice
...
void wakeup_all_seller_threads() {
	...
	if(verbose)
		printf("00:%02d Main Thread Clock Tick\n",sim_time); 
	pthread_cond_broadcast( &condition_cond);
	pthread_mutex_unlock( &condition_mutex );
}

void *sell(void *t_args) { //Initializing thread
	...
	while(sim_time < simulation_duration) { 	//Waiting for clock tick
